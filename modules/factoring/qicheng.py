...
def qicheng(stop,n,timeout,args):
	try:
...
class KThread(threading.Thread):
	...
	def run(this):
		try:
			p = q = None
			p = os.popen("sage %s/sage/qicheng.sage %s"%(this.rootpath,str(this.n))).read().strip()
			q = this.n // int(p)
			if(int(p)*q != this.n or p==1 ):
				return None
			else:
				if(not this.args.json):
					if(this.args.verbose):
						logger("[+] qicheng attack found",'log',1,0)
					...
					print('{"method":"qicheng","factor":[%s,%s]}'%(str(p),str(q)))
