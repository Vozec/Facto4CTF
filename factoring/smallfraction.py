...
def smallfraction(stop,n,timeout,args):
	try:
...
class KThread(threading.Thread):
	...
	def run(this):
		try:
			...
			p = q = None
			p = int(os.popen("sage %s/sage/smallfraction.sage %s"%(this.rootpath,str(this.n))).read())
			q = this.n // p
			...
			if(p*q != this.n or p==1 ):
				...
				if(not this.args.json):
					if(this.args.verbose):
						logger("[+] smallfraction attack found",'log',1,0)
					...
					print('{"method":"smallfraction","factor":[%s,%s]}'%(str(p),str(q)))
